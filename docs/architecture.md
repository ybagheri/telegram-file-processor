...
**Pre-download guards** (in `worker.py`, before `telegram_service.download(...)` runs): a job whose *declared* Telegram size exceeds `Processing.MAX_FILE_SIZE` is rejected with a `Protocol.create_error(...)` through the bridge (Persian message to the user) and `job.cleanup()` — for multi-volume jobs the sum of all parts' declared sizes is checked, so the set can't be smuggled in piece by piece. Second, a free-disk-space guard (`_has_enough_disk_space`): for declared sizes above `DISK_SPACE_CHECK_THRESHOLD`, every one of `Paths.DOWNLOADS`/`Paths.TEMP`/`Paths.OUTPUTS` must have `declared_size × DISK_SPACE_SAFETY_FACTOR` free (`shutil.disk_usage`), or the job is rejected the same way. This deliberately pre-checks only "input + a safety factor of working space" — it does *not* try to reserve room for a fully-expanded archive, which would defeat the disk-conscious streaming approach in `ArchiveProcessor` (extract-one-file-at-a-time, delete before the next).

**Concurrency:** the heavy processing step (ffmpeg/extraction) of every job — single-file and multi-volume alike — runs under a shared `asyncio.Semaphore` sized by `MAX_CONCURRENT_JOBS` (default 2). Downloads are not bounded by it; queued jobs acquire the semaphore in arrival order, and a job that raises releases its slot immediately.
...
**URL-upload jobs** (job payload carries `url` instead of `message_id`): the user sent a direct http(s) link instead of a file. `handlers/core.py::handle_url_submission` validates the link (scheme whitelist, SSRF guard in `utils/url_validation.py` — IP literals and DNS-resolved addresses in private/loopback/link-local ranges are rejected, rate limit applies) and registers a normal `PendingFile` with `url` set, so the quality/options/target flow is byte-for-byte the shared one; `finalize_job` just skips the media forward and puts the URL in the job payload. On the worker, `process_url_job` streams the file from the URL to the job's input dir (`services/url_downloader.py`, stdlib urllib in a worker thread) with `MAX_FILE_SIZE` enforced twice — `Content-Length` up front and a hard cap while streaming — then applies the same size/disk guards and the same dispatch/delivery pipeline as an uploaded file. The only new piece is "get bytes onto disk from a URL"; everything downstream is reused unchanged.
...
**Command menus:** on startup `bot.py::register_command_menus` calls `setMyCommands` — `utils/bot_commands.py`'s public set (`/start`, `/settings`, `/cancel`) under `BotCommandScopeDefault`, and the admin set (`/admin`, `/status`) plus the public set under `BotCommandScopeChat` for each configured admin id (a chat scope replaces the default menu, so the public commands must be repeated there). README's command table is the human-facing mirror of these lists and is required maintenance alongside them.
